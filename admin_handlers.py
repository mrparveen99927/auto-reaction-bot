...
async def gen_key(update: Update, context):
    ...
    try:
        if not context.args or len(context.args) < 2:
            ...
            return
        new_id = str(context.args).strip()
        new_pass = str(context.args).strip()
        days = int(context.args) if len(context.args) > 2 else 30
...
async def rem_key(update: Update, context):
    ...
    try:
        if not context.args:
            ...
            return
        target_id = str(context.args).strip()
...
async def users_list(update: Update, context):
    ...
    for row in rows:
        reply += f"🔑 ID: `{row}` | P: `{row}`\n📢 Group: `{row if row else 'None'}`\n⏳ Exp: `{row}`\n──────────────────\n"
